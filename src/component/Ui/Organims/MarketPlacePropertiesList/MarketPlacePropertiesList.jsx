            ...
            <div className="w-10/12 grid grid-cols-3 gap-8 mx-auto relative z-40 ">
                {
                    marketPlaceList.map((item, index) => (
                        <Link to={`/Product/${item.id}`} key={index} className="dark:bg-[#17191C] flex flex-col gap-6 items-center bg-white border-2 border-[#DCDCEB] rounded-3xl p-[1.5vw] hover:border-[#1DAEFF] hover:shadow-lg hover:shadow-gray-300 scale-100 transform hover:scale-105 transition-transform duration-300 ease-in-out">
                            ...
                            <div className="flex flex-col gap-5 items-start justify-between w-full p-[0.5vw]">
                                ...
                                <div className="w-full pt-2 flex justify-between ">
                                    ...
                                    <P className="p-2 px-4 bg-[#8F90A6] text-white text-[clamp(0.7rem,1.2vw,1.8rem)] font-semibold rounded-lg">{item.price}</P>
                                </div>
                            </div>
                        </Link>
                    ))
