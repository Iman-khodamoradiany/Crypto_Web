...
function SubmitLoginForm({ setCheck, check }) {
    ...
    return (
        <div className="w-full flex justify-center items-center flex-col gap-4">
            ...
            <div className="flex justify-center items-center text-[1vw] gap-2">
                <P className={"text-[#8F90A6]"}>{check ? 'I have account' : 'Don’t you have account ?'}</P>
                <Button type={'button'} className="text-[#1DAEFF]" onClick={() => setCheck(!check)}>{check ? "Login" : "Sign up"}</Button>
            </div>
        </div >
    )
}
